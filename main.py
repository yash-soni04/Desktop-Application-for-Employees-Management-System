...
def Add():
    ...
    coursename = e3.get()
    feee = e4.get()
    mysqldb=mysql.connector.connect(host="localhost",user="root",password="",database="emp_management_system")
    mycursor=mysqldb.cursor()
    try:
       sql = "INSERT INTO employee_details (id,empname,mobile,salary) VALUES (%s, %s, %s, %s)"
...
def update():
    ...
    mycursor=mysqldb.cursor()
    try:
       sql = "Update employee_details set empname= %s,mobile= %s,salary= %s where id= %s"
...
def show():
        mysqldb = mysql.connector.connect(host="localhost", user="root", password="", database="emp_management_system")
        mycursor = mysqldb.cursor()
        mycursor.execute("SELECT id,empname,mobile,salary FROM employee_details")
...
global e3
global e4
tk.Label(root, text="Employee  employee_details", fg="red", font=(None, 30)).place(x=300, y=5)
